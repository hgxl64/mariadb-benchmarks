...
collect_server_stats() {
    ...
    if [[ IS_MARIADB ]] && cat /proc/\$(pidof mariadbd)/status > ${LOGDIRECTORY}/${prefix}_mariadbd_status.txt
    if [[ IS_MYSQL ]]   && cat /proc/\$(pidof mysqld)/status   > ${LOGDIRECTORY}/${prefix}_mysqld_status.txt
