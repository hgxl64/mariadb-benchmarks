...
CREATE TABLE sbseq (
  ...
  PRIMARY KEY (id)
   ...
   query = [[
CREATE FUNCTION next_val(seq_id INT UNSIGNED)
RETURNS BIGINT UNSIGNED
NOT DETERMINISTIC
MODIFIES SQL DATA
SQL SECURITY INVOKER
BEGIN
  UPDATE sbseq SET val=LAST_INSERT_ID(val+1) WHERE id=seq_id;
...
###########-###########-###########-###########-###########-###########-###########-###########-###########-###########]])
         pad_val = sb_rand_str([[
###########-###########-###########-###########-###########]])
         rs = db_query("INSERT INTO " .. table_name ..  " (id, k, c, pad) VALUES " .. string.format("(next_val(1), %d, '%s', '%s')", k_val, c_val, pad_val))
      end
